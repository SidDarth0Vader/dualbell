...
while flag == 0:
        ...
        for x in range(nop):
            if pts[x].pointloc == 'wall':
                if ExMa>=3*OG_ExMa:
                    flag = 1
                    truncFlag = 0
                    print("Unable to generate a TICN")
                    break
                elif abs(pts[x].x-nozzLength) <= 0.05:
    ...
    elif pts[noc-1].x >= nozzLength:
        flag = 1
        truncFlag = 0
        print("Unable to generate a TICN")
    else:
        print("\nIdeal Contour Nozzle is shorter than the entered nozzle length,\ngenerating ideal contour nozzle instead...\n")
        flag = 1
    count+=1
    ExMa+=0.01
...
if truncFlag == 0:
    ...
    pts = []
    tmp = noc + 1
    infPtLocX = downWallRad * np.sin(np.radians(thetamax + Nu(ExMa, rosh)))
    infPtLocY = th_rad + downWallRad - downWallRad * np.cos(np.radians(thetamax + Nu(ExMa, rosh)))
    for x in range(nop):
        #print(x, tmp, x - tmp, x - 1)
        if ptloc_array[x] == 'axis' and x>noc and x>0:
            tmp-=1
...
def TICNlen(): #This function draws the ideal contour nozzle
    conx = []
    cony = []
...
def altDrawChar():
    ...
    for h in range(nop):
        ...
        elif pts[h].pointloc == 'wall' and pts[h].pno != pts[nop-1].pno:
            mat.plot([pts[h-back].x, pts[h].x], [pts[h-back].y, pts[h].y], colour)
            back -= 1

        elif pts[h].pno == pts[nop-1].pno:
            mat.plot([pts[h - back].x, nozzLength], [pts[h - back].y, pts[h].y], colour)
...
altDrawChar()
# drawChar()
TICNlen()      # This function draws the minimum length nozzle
