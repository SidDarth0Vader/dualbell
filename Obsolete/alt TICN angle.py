...
class PredetPoints:
    def __init__(self, aloc_pno):
        self.pno = aloc_pno+1
        self.x = downWallRad*np.sin(np.radians((self.pno/noc)*throatAng))
        self.y = th_rad+downWallRad - downWallRad*np.cos(np.radians((self.pno/noc)*throatAng))
        for x in range(len(thetas)):
            if (round(self.x,4) == round(conx[x],4)) and (self.pno != noc):
                self.theta = thetas[x]
            elif self.pno == noc:
                self.theta = throatAng
        self.aR = self.y**2/th_rad**2
        self.Ma = 0
        for x in range(len(aRtab)):
            if abs(aRtab[x]-self.aR) <= 0.0001:
                self.Ma = M_array[x]
...
prepts = []
while i < noc:
    prepts.append(PredetPoints(i))
    i += 1

for x in range(len(prepts)):
    print("P no. = ", prepts[x].pno, "theta = ", prepts[x].theta, "Mu = ", prepts[x].mu, "Th-Mu = ", (prepts[x].theta-prepts[x].mu), prepts[x].Kmin)
...
class Points:
    def __init__(self, aloc_pno, char1, char2):
        ...
        self.y = 0
        if self.pno == 1:
            self.Kmin = prepts[self.pno-1].Kmin
...
# Alternate method wall point plot
        if self.pointloc == 'wall' and self.pno == noc+1:
            ...
            x2 = char2.x
            y2 = char2.y
            m1 = np.tan(np.radians((char1.theta+self.theta)/2))
            m2 = np.tan(np.radians((char2.theta+self.theta)*0.5+(char2.mu+self.mu)*0.5))
            denom = 1 - (m1/m2)
            self.x = (x2 + (1/m2)*(y1 - y2 - x1*m1))/denom
            #self.y = np.sqrt(self.aR * (th_rad ** 2))
...
# Characteristic drawing
        if self.pno == 1:
            self.x = prepts[self.pno-1].x-prepts[self.pno-1].y/np.tan(np.radians(prepts[self.pno-1].theta-prepts[self.pno-1].mu))
            # print(prepts[self.pno-1].y)
            # print(prepts[self.pno-1].theta)
            # print(prepts[self.pno-1].mu)
            # print('x = ', self.x)

        elif self.pointloc == 'axis' and self.pno != 1:
            self.x = char1.x-char1.y/np.tan(np.radians(char1.theta-char1.mu))
            # print(char1.pno)
            # print(char1.theta)
            # print(char1.mu)

        elif self.pointloc == 'interior' and self.pno <= noc:
            x1 = prepts[self.pno-1].x
            y1 = prepts[self.pno-1].y
            x2 = char2.x
            y2 = char2.y
            m1 = np.tan(np.radians((prepts[self.pno-1].theta+self.theta)*0.5-(prepts[self.pno-1].mu+self.mu)*0.5))
...
def drawChar():
    ...
    tmp = noc+1
    for h in range(nop):
        if ptloc_array[h] == 'axis' and h>noc and h>0:
            tmp -= 1
            mat.plot([pts[h-tmp].x, pts[h].x],[pts[h-tmp].y,pts[h].y], colour, linewidth = 0.5)

        elif pts[h].pno <= noc:
            mat.plot([prepts[h].x, pts[h].x], [prepts[h].y, pts[h].y], colour, linewidth = 0.5)
